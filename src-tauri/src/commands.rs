...
pub async fn edit_medicine(
    ...
    if let Some(bn) = batch_number {
        update_doc.insert("batch_number", bn);
    }
    if let Some(ed) = expiry_date {
        update_doc.insert("expiry_date", ed);
    }
    if let Some(q) = quantity {
        update_doc.insert("quantity", q);
    }
    if let Some(pp) = purchase_price {
        update_doc.insert("purchase_price", pp);
    }
...
#[command]
pub async fn delete_medicine(medicine_id: String, hospital_id: String) -> Result<String, String> {
    let db = get_db_connection().await;
    let collection: Collection<Medicine> = db.collection("medicines");
...
#[command]
pub async fn get_stock(hospital_id: String) -> Result<Vec<Medicine>, String> {
    let db = get_db_connection().await;
    let collection: Collection<Medicine> = db.collection("medicines");
